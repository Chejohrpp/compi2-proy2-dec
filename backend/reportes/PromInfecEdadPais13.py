...
def analizar(filepath,param):
    ...
    x_celda = param['celda_edad']
    y_celda = param['celda_fallecidos']
    y_celda2 = param['celda_confirmados']
    ...
    #### Muertes promedios ##########################################################
    datos_calculados.append(" Muertes promedios: ")
    datos_calculados.append(" edad,confirmados -> fallecidos ")
    for i in range(len(df_xcelda)):
        aux_edad  = df_xcelda.iloc[i]
        aux_confirm = round(y_predictions2[i],2)
        aux_dead = round(y_predictions[i],2)
        datos_calculados.append("{} , {} -> {}".format(aux_edad,aux_confirm,aux_dead))
    #### Graph #######################################################################
    ...
    plt.clf()
    #### enviar los datos #######################################################################
    return fr.addData(datos_calculados,lista_urls_imgs,lista_urls_static,datos_estaticos,'graficas de relacion con {} y {} en base a {}'.format(y_celda,y_celda2,x_celda),name)
